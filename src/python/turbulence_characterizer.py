...
def estimate_tau0_temporal_structure_function(signal: np.ndarray,
    ...
    fit_line = KOLMOGOROV_CONST * (v_over_r0 * (lags_ms * 1e-3) / r0_m) ** (5 / 3)
...
class TurbulenceCharacterizer:
    ...
    def run(self,
        ...
        s2 : dict | None   Stage 2 results (zernike_coeffs if modal)
        ...
        # ── r0, Method A (only if Zernike coefficients are available) ──────
        zern_var = zern_theory = zern_modes = None
        r0_zernike = float("nan")
        signal_used = "mean tip-tilt proxy (no Zernike coeffs available)"
        ...
        if s2 is not None and "zernike_coeffs" in s2 and s2["zernike_coeffs"].ndim == 2:
            ...
            temporal_signal = coeffs[:, 0]   # j=2, tip
            signal_used = "Zernike tip mode (j=2)"
        else:
            # Fallback: mean x-slope across valid SAs, per frame
        ...
        # ── tau0 estimates (use whichever r0 is more reliable: zernike if
        #    available, else slope-SF) ─────────────────────────────────────
        r0_for_tau = r0_zernike if not np.isnan(r0_zernike) else r0_sf
